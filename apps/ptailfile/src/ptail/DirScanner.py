        ...
        for path, subpaths, files in os.walk(self.root, True, on_error, False):
            ...
            if depth < max_depth:
                ...
                for subp in subpaths:
                    if fnmatch.fnmatchcase(subp, regx) is False:
                ...
                for file in files:
                    ...
                    #change > to >=
                    if fnmatch.fnmatchcase(file, regx) and os.path.getmtime(full_path) >= self.modify_time:
...
if __name__ == '__main__':
    dir_scanner = DirScanner("D:\\workspace\\ptailfile\\test", "null", 0, "d2\\*", ["1", "2", 'D:\\workspace\\ptailfile\\test\\d2\\d2f.txt'])

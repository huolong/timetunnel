...
class Config:
    ...
    def __init_topic(self):
        ...
        for name in self.ret:
            topics.append(name.replace("topic_", ""))
    ...
    def __init_passport(self):
        value = self.config.get('tt2', 'passport')
        if value is None or value is "":
    ...
    def __init_routers(self):
        value = self.config.get('tt2', 'routers')
        if value is None or value is "":
    ...
    def __init_log_base_path(self):
        value = self.config.get('app', 'log_base_path')
        if value is None or value is "":
            logger.error("no log_base_path defined in conf " + self.confFile + +" and use default")
            sys.exit(1)
